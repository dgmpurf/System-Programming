...
int isEmpty(MINODE *mip)
{
	...
	{
		//link count = 2 could still have files, check data block(s)
		if(ip->i_block[1])
		{
...
int rmdir()
{
    ...
    ino = getino(pathname);
	//(3)get its minodep[] pointer
    mip = iget(dev, ino);

    //(4)check user id or super user
	//super user: OK
    ...
    iput(mip);//which clears mip->reCount = 0

    //(7)get parent DIR's ino and minode
	...
	//(9)decrement pip link_count by 1
	//touch pip atime, mtime fields
	//mark pip dirty
...
int rm_child(MINODE *parent, char *name)
{
	...
	for(i = 0; i < 12 ; i++)
	{
		...
		while(cp < buf + 1024)
		{
			...
			printf("dp is at %s\n", temp);
			//check if child is found, compare it
			if(!strcmp(temp, name))
			{
				printf("child found!\n");
				...
				//if first entry in data block
				else if(cp == buf && cp + dp->rec_len == buf + 1024)
				{
				...
				//parents data block back to disk
